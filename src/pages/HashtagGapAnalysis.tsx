...
import {
  ...
  Sparkles, Hash, ArrowRight, Info, Wifi, History, ChevronsRight,
} from "lucide-react";
          ...
          <div className="flex items-center justify-between">
            ...
            {portfolio.length > 0 && (
              <div className="relative" ref={portfolioRef}>
                ...
                {showPortfolio && (
                  <div className="absolute right-0 top-full mt-2 w-72 bg-card border border-border rounded-xl shadow-xl z-50 overflow-hidden animate-fade-in">
                    <div className="flex items-center justify-between px-3 py-2.5 border-b border-border">
                      <p className="text-xs font-semibold text-foreground">Your analysis history tags</p>
                      <span className="text-[10px] text-muted-foreground">{portfolio.length} unique tags</span>
                    </div>
                    <div className="max-h-52 overflow-y-auto p-2 space-y-0.5">
                      {portfolio
                        .sort((a, b) => b.avg_score - a.avg_score)
                        .map((t) => (
                          <div key={t.tag} className="flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-secondary/60 transition-colors">
                            <span className="text-xs font-mono text-foreground">{t.tag}</span>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] text-muted-foreground">{t.uses}× used</span>
                              <span className={`text-[10px] font-bold tabular-nums ${
                                t.avg_score >= 70 ? "text-emerald-400" :
                                t.avg_score >= 50 ? "text-amber-400" : "text-muted-foreground"
                              }`}>{t.avg_score}</span>
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="p-2 border-t border-border">
                      <button
                        onClick={() => {
                          setYourInput(portfolio.map((t) => t.tag).join(" "));
                          setShowPortfolio(false);
                          toast.success("History tags loaded into 'Your hashtags'");
                        }}
