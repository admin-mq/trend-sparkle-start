    ...
    const actions = [
      {
        ...
        steps: JSON.stringify(["Rewrite title tag to match searcher intent", "Add compelling meta description with CTA", "Test with A/B title variations"]),
        severity: "high",
      },
      {
        snapshot_id: snapshotId,
        ...
        steps: JSON.stringify(["Add JSON-LD Product schema", "Include price, availability, and review data", "Validate with Google Rich Results Test"]),
        severity: "high",
      },
      {
        snapshot_id: snapshotId,
        ...
        steps: JSON.stringify(["Audit all category pages for missing meta descriptions", "Write unique, keyword-rich descriptions under 160 chars", "Deploy and monitor snippet changes in GSC"]),
        severity: "medium",
      },
      {
        snapshot_id: snapshotId,
        ...
        steps: JSON.stringify(["Audit canonical tags across category pages", "Fix self-referencing canonical issues", "Remove conflicting canonical declarations"]),
        severity: "medium",
      },
      {
        snapshot_id: snapshotId,
        ...
        steps: JSON.stringify(["Map current link depth for all key pages", "Add contextual internal links from high-authority pages", "Create hub pages for orphaned content"]),
        severity: "low",
      },
    ];
